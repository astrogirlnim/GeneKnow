...
=======
    ...
    def create_architecture_diagrams(self):
        ...
        # Draw static models
        for model in static_models:
            rect = plt.Rectangle((1, model['y']-0.4), 3, 0.8, 
                               facecolor=model['color'], 
                               edgecolor='black', 
                               linewidth=2)
            ax.add_patch(rect)
            ax.text(2.5, model['y'], model['name'], 
                   ha='center', va='center', fontweight='bold', fontsize=11)
            ax.text(2.5, model['y']-0.25, model['desc'], 
                   ha='center', va='center', fontsize=8, style='italic')
            
            # Draw arrows to fusion layer
            ax.arrow(4.2, model['y'], 2.6, 5-model['y'], 
                    head_width=0.2, head_length=0.2, 
                    fc='gray', ec='gray', alpha=0.7)
        
        # Draw fusion layer
        fusion_rect = plt.Rectangle((7, 4), 4, 2, 
                                  facecolor='gold', 
                                  edgecolor='black', 
                                  linewidth=3)
        ax.add_patch(fusion_rect)
        ax.text(9, 5, 'ML Fusion Layer', 
               ha='center', va='center', fontweight='bold', fontsize=14)
        ax.text(9, 4.5, 'Gradient Boosting', 
               ha='center', va='center', fontsize=10, style='italic')
        
        # Draw output
        ax.arrow(11.2, 5, 1.6, 0, 
                head_width=0.3, head_length=0.2, 
                fc='green', ec='green', linewidth=2)
        
        risk_rect = plt.Rectangle((13, 4.5), 3, 1, 
                                facecolor='lightgreen', 
                                edgecolor='black', 
                                linewidth=2)
        ax.add_patch(risk_rect)
        ax.text(14.5, 5, 'Risk Score', 
