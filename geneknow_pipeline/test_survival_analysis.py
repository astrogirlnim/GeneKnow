...
def test_survival_analysis_generation():
    ...
    for cancer_type, curve_data in survival_curves.items():
        ...
        if len(time_points) != len(pop_survival) or len(time_points) != len(
            pat_survival
        ):
        ...
        print(
            f"✅ {cancer_type}: {len(time_points)} time points, HR={curve_data['hazard_ratio']}"
        )
...
def test_api_data_structure():
    ...
    for field, expected_type in expected_structure.items():
        ...
        if not isinstance(survival_analysis[field], expected_type):
            print(
                f"❌ Wrong type for {field}: expected {expected_type}, got {type(survival_analysis[field])}"
            )
            return False
    ...
    for cancer_type, curve_data in survival_curves.items():
        ...
        for field, expected_type in required_fields.items():
            ...
            if not isinstance(curve_data[field], expected_type):
                print(
                    f"❌ Wrong type for {cancer_type}.{field}: expected {expected_type}, got {type(curve_data[field])}"
                )
                return False
