...
def generate_survival_curve(base_survival: float, hazard_ratio: float, 
...
def generate_confidence_bands(survival_curve: np.ndarray, 
...
def process(state: Dict) -> Dict:
    ...
    try:
        ...
        mutations = list(set(v.get('gene') for v in variants 
                           if v.get('gene') in MUTATION_HAZARD_RATIOS))
        
        # Extract disrupted pathway IDs
        pathway_ids = [p.get('pathway_id') for p in disrupted_pathways 
