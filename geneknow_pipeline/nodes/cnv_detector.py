...
def process(state: Dict) -> Dict:
    ...
    try:
        ...
        for gene in genes_to_check:
            ...
            has_cnv = (variant_count >= 3 or 
                      (gene in CANCER_CNVS and variant_count >= 1) or
                      (gene in CANCER_CNVS and np.random.random() < 0.3))  # 30% chance for known genes
        ...
        significance_order = {'pathogenic': 0, 'likely_pathogenic': 1, 
                            'uncertain_significance': 2, 'benign': 3}
        ...
        cnv_summary = {
            ...
            'amplifications': sum(1 for cnv in annotated_cnvs 
                                if cnv.get('type') == 'amplification'),
            'deletions': sum(1 for cnv in annotated_cnvs 
                           if cnv.get('type') in ['heterozygous_deletion', 'homozygous_deletion']),
            'pathogenic_cnvs': sum(1 for cnv in annotated_cnvs 
                                 if cnv.get('clinical_significance') in ['pathogenic', 'likely_pathogenic']),
            'cancer_associated': sum(1 for cnv in annotated_cnvs 
                                   if 'cancer_relevance' in cnv),
