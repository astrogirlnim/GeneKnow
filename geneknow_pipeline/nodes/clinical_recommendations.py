...
def generate_prevention_recommendations(risk_scores: Dict, pathways: List[Dict]) -> Dict:
    ...
    dna_repair_disrupted = any(p.get('pathway_id') == 'DNA_REPAIR' 
                              for p in pathways if p.get('significance', 0) > 50)
...
def generate_clinical_trials_recommendations(variants: List[Dict], pathways: List[Dict]) -> List[str]:
    ...
    targetable_genes = set(v.get('gene') for v in variants 
                          if v.get('gene') in TARGETED_THERAPIES)
    ...
    immune_pathway_disrupted = any(p.get('pathway_id') == 'IMMUNE_CHECKPOINT' 
                                  for p in pathways)
    ...
    dna_repair_disrupted = any(p.get('pathway_id') == 'DNA_REPAIR' 
                              for p in pathways if p.get('significance', 0) > 30)
...
def generate_priority_actions(screening_recs: List[Dict], 
                            therapy_recs: List[Dict], 
                            prevention_recs: Dict) -> List[str]:
