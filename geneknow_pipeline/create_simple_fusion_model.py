...
def create_simple_fusion_model():
    ...
    for i in range(n_samples):
        ...
        inputs = StaticModelInputs(
            ...
            clinvar_classification=np.random.choice(['pathogenic', 'benign', 'uncertain', 'not_found']),
    ...
    metrics = fusion_layer.train(training_data, validation_split=0.0)
    ...
    with open(os.path.join(output_dir, "gradient_boosting_class_weight.pkl"), 'wb') as f:
    ...
    with open(os.path.join(output_dir, "feature_columns.pkl"), 'wb') as f:
        feature_names = fusion_layer.feature_names + [f'clinvar_{cat}' for cat in fusion_layer.clinvar_categories]
    ...
    metadata = {
        ...
        "feature_count": len(fusion_layer.feature_names) + len(fusion_layer.clinvar_categories),
    ...
    print(f"   SHAP validation should now work!")
