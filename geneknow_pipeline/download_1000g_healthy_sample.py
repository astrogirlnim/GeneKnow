                ...
                for variant in data[:10]:  # Limit to first 10 per gene
                    if 'MAF' in variant and variant['MAF'] > 0.01:  # Common variants
                        all_variants.append({
                            ...
                            'ref': variant.get('ancestral_allele', 'A'),
                            'alt': 'G',  # Simplified
                            'maf': variant.get('MAF', 0.05),
    ...
    variants = [
        {
            ...
            'ref': 'A',
            'alt': 'G',
            'maf': 0.12,  # 12% population frequency
...
def save_as_simplified_vcf(variants):
    ...
    output_file = "test_validation_data/healthy_1000g_sample.vcf"
    ...
    with open(output_file, 'w') as f:
        ...
        for var in variants:
            ...
            format_field = "GT"
            genotype = "0/1" if var['maf'] > 0.05 else "0/0"
...
def convert_vcf_to_maf(vcf_file):
    ...
    maf_file = "test_validation_data/healthy_1000g_sample.maf"
