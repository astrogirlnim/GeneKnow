...
from nodes.ml_fusion_node import MLFusionNode
...
def test_direct_pipeline():
    ...
    test_files = [
        ...
        "test_data/tcga_downloads/test_sample.maf",
        "../test_data/test.vcf"
    ]
    ...
    if prs_results:
        ...
        for cancer, prs_data in prs_results.items():
            print(f"      {cancer}: score={prs_data.get('raw_score', 0):.3f}, percentile={prs_data.get('percentile', 0)}%")
    else:
    ...
    if tcga_summary:
        ...
        if enrichment:
            print(f"      Max enrichment: {enrichment.get('enrichment', 0):.1f}x in {enrichment.get('cancer_type', 'N/A')}")
    else:
...
def test_api_flow():
    ...
    # Find test file
    test_file = None
    for f in ["test_data/242b87b1-bf7c-4c1a-bed2-bb077e5ccd00.wxs.aliquot_ensemble_masked.maf.gz", 
              "test_data/test_variants.vcf", 
              "test_data/tcga_downloads/test_sample.maf"]:
        if os.path.exists(f):
    ...
    response = requests.post(f"{api_url}/api/process", json={
        "file_path": test_file,
        "preferences": {
